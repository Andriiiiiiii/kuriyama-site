/**
 * Global Typography Configuration
 *
 * This file contains all font size configurations for the entire site.
 * Font sizes are calculated based on viewport width using the formula:
 * fontSize = calc(baseFontSize / baseWidth * 100vw)
 *
 * To adjust font sizes globally:
 * 1. Change the base font size values below
 * 2. Change the baseWidth if needed (default: 1441.5px)
 */

export const TYPOGRAPHY_CONFIG = {
  // Base width for normalization (px)
  baseWidth: 1441.5,
  // Base height for normalization (px)
  baseHeight: 1088,

  // Font sizes in pixels (at baseWidth)
  fontSizes: {
    // Main heading font size (80px at 1441.5px width)
    heading: 70,

    // Subheading font size (30px at 1441.5px width)
    subheading: 30,

    // Large text font size (40px at 1441.5px width)
    large: 40,

    // Medium text font size (25px at 1441.5px width) - between subheading and body
    medium: 25,

    // Regular text font size (22px at 1441.5px width)
    body: 20,

    // Small text font size (15px at 1441.5px width)
    small: 15,

    // Giant heading for poster-like slides
    giant: 170,
  },

  // Line height ratios (unitless) at baseHeight
  lineHeight: {
    base: 1.2,
    min: 1,
    max: 1.4,
  },

  // Decorative line stroke width relative to viewport width
  // At 1920px width: 0.05vw ≈ 0.96px
  lineStrokeWidth: '0.05vw',

  // Line thicknesses in pixels (at baseWidth)
  lineThickness: {
    hairline: 1,
    thin: 2,
    medium: 4,
    thick: 6
  }
} as const;

/**
 * Generate CSS calc() string for responsive font size
 * @param fontSize - Font size in pixels at baseWidth
 * @returns CSS calc() string
 */
export const getFontSize = (fontSize: number): string => {
  return `calc(${fontSize} / ${TYPOGRAPHY_CONFIG.baseWidth} * 100vw)`;
};

/**
 * Generate CSS calc() string for responsive line thickness,
 * ensuring a minimum of 1px to avoid rendering inconsistencies.
 * @param thickness - Thickness in pixels at baseWidth
 * @returns CSS string using max()
 */
export const getLineThickness = (thickness: number): string => {
  // Use max(1px, responsive_value) to prevent lines from becoming too thin or disappearing
  return `max(1px, calc(${thickness} / ${TYPOGRAPHY_CONFIG.baseWidth} * 100vw))`;
};

/**
 * Calculate actual pixel line thickness for a given window width
 * Rounds to nearest integer and ensures min 1px.
 * Used for precise pixel-alignment in JS.
 * 
 * @param thickness - Base thickness in pixels (at baseWidth)
 * @param windowWidth - Current window width in pixels
 * @returns Calculated thickness in pixels (integer)
 */
export const calculateLineThickness = (thickness: number, windowWidth: number): number => {
  const scaled = (thickness / TYPOGRAPHY_CONFIG.baseWidth) * windowWidth;
  return Math.max(1, Math.round(scaled));
};

/**
 * Calculate responsive line-height ratio based on viewport height
 * @param height - Viewport height in pixels
 * @returns Line-height ratio (unitless)
 */
export const getLineHeight = (height: number): number => {
  const { baseHeight, lineHeight } = TYPOGRAPHY_CONFIG;
  const scaled = (height / baseHeight) * lineHeight.base;
  return Math.min(lineHeight.max, Math.max(lineHeight.min, scaled));
};

/**
 * Get line-height ratio for current viewport height
 */
export const getViewportLineHeight = (): number => {
  if (typeof window === 'undefined') {
    return TYPOGRAPHY_CONFIG.lineHeight.base;
  }
  return getLineHeight(window.innerHeight);
};

/**
 * Pre-calculated font size utilities
 */
export const FONT_SIZES = {
  heading: getFontSize(TYPOGRAPHY_CONFIG.fontSizes.heading),
  subheading: getFontSize(TYPOGRAPHY_CONFIG.fontSizes.subheading),
  large: getFontSize(TYPOGRAPHY_CONFIG.fontSizes.large),
  medium: getFontSize(TYPOGRAPHY_CONFIG.fontSizes.medium),
  body: getFontSize(TYPOGRAPHY_CONFIG.fontSizes.body),
  small: getFontSize(TYPOGRAPHY_CONFIG.fontSizes.small),
  giant: getFontSize(TYPOGRAPHY_CONFIG.fontSizes.giant),
} as const;

/**
 * Pre-calculated line thickness utilities
 */
export const LINE_THICKNESS = {
  hairline: getLineThickness(TYPOGRAPHY_CONFIG.lineThickness.hairline),
  thin: getLineThickness(TYPOGRAPHY_CONFIG.lineThickness.thin),
  medium: getLineThickness(TYPOGRAPHY_CONFIG.lineThickness.medium),
  thick: getLineThickness(TYPOGRAPHY_CONFIG.lineThickness.thick),
} as const;

/**
 * Centralized font-family tokens for all pages.
 */
export const FONT_FAMILIES = {
  heading: "UA-brand, Impact, sans-serif",
  body: "Glametrix, Inter, sans-serif",
  accent: "Han-Zi, sans-serif",
  ui: "Inter, sans-serif",
} as const;
